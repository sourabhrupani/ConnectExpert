...
{
	var startval=startpath ? startpath.split("http://www.question2answer.org/") : [];
	var setdescnow=true;
								...
								if (lines[0]=='1') {
									...
									if (lines.length>2) {
										...
										for (var i=2; i<lines.length; i++) {
											var parts=lines[i].split('http://www.question2answer.org/');

											if (String(qa_cat_exclude).length && (String(qa_cat_exclude)==parts[0]))
												continue;

											newelem.options[newelem.options.length]=new Option(parts.slice(1).join('http://www.question2answer.org/'), parts[0]);
											addedoption=true;
										}
